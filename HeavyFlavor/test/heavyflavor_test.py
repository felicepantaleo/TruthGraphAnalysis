...
import heavyflavor  # noqa: E402


class TestHeavyFlavor(unittest.TestCase):
    def test_matches_the_expected_output(self):
        graph = TruthGraphView.fromJson(os.path.join(HERE, "..", "..", "Common", "fixtures", "top.json"))
        out = io.StringIO()
        heavyflavor.run(graph, out)
