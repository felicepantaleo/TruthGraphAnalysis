...
class TestHeavyFlavor : public CppUnit::TestFixture {
  CPPUNIT_TEST_SUITE(TestHeavyFlavor);
...
public:
  ...
  void testMatchesTheExpectedOutput() {
    ...
    tga::heavyflavor::run(graph, out);
    std::ifstream expectedFile(path("HeavyFlavor/test/expected.txt"));
...
CPPUNIT_TEST_SUITE_REGISTRATION(TestHeavyFlavor);
