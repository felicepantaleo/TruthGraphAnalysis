...
class TestTop : public CppUnit::TestFixture {
  CPPUNIT_TEST_SUITE(TestTop);
...
public:
  ...
  void testMatchesTheExpectedOutput() {
    ...
    tga::top::run(graph, out);
    std::ifstream expectedFile(path("Top/test/expected.txt"));
...
CPPUNIT_TEST_SUITE_REGISTRATION(TestTop);
