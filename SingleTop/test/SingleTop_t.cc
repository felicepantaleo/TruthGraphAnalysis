...
class TestSingleTop : public CppUnit::TestFixture {
  CPPUNIT_TEST_SUITE(TestSingleTop);
...
public:
  ...
  void testMatchesTheExpectedOutput() {
    const truth::Graph graph = tga::graphFromJson(path("Common/fixtures/singletop.json"));
    std::ostringstream out;
    tga::singletop::run(graph, out);
    std::ifstream expectedFile(path("SingleTop/test/expected.txt"));
...
CPPUNIT_TEST_SUITE_REGISTRATION(TestSingleTop);
